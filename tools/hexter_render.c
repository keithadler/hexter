/* hexter-render: render DX7 notes or a MIDI file to a WAV file, no host needed
 *
...
{
    fprintf(stderr,
        "hexter-render %s: render DX7 sounds to a WAV file\n"
        "\n"
        "  hexter-render [options] --note N [--note N ...]\n"
        "  hexter-render [options] --midi FILE.mid\n"
        "  hexter-render --bank FILE --list\n"
        "\n"
        "options:\n"
        "  --bank FILE       DX7 bank (.syx, .dx7, .mid, .tx7, .snd, .bnk, .dx2, raw)\n"
        ...
        "  --volume DB       -70..20 (default 0)\n"
        "  --poly N          polyphony 1-64 (default 10)\n"
        ...
        "  --list            print the program names in the bank and exit\n",
        HEXTER_ENGINE_VERSION);
}
...
main(int argc, char **argv)
{
    ...
    int program = 1, velocity = 100, poly = HEXTER_ENGINE_DEFAULT_POLYPHONY, mono = 0, list = 0;
    int notes[64], nnotes = 0;
    double hold = 2.0, seconds = -1.0, tail = 3.0, tuning = 440.0, volume = 0.0;
    uint32_t rate = 48000;
    hexter_engine_t *e;
    ...
    for (i = 1; i < argc; i++) {
        ...
        else if (!strcmp(a, "--volume") && v) { volume = atof(v); i++; }
    ...
    e = hexter_engine_new((float)rate);
    if (!e) { fprintf(stderr, "could not create engine\n"); return 1; }

    if (bank) {
        char *err = NULL;
        int n = hexter_engine_load_bank_file(e, bank, 0, &err);
        if (!n) {
    ...
    if (list) {
        char name[11];
        for (i = 0; i < HEXTER_BANK_PATCHES; i++) {
            hexter_engine_get_program_name(e, i, name);
            printf("%3d  %s\n", i + 1, name);
        }
        hexter_engine_free(e);
        return 0;
    ...
    hexter_engine_set_tuning(e, (float)tuning);
    hexter_engine_set_volume(e, (float)volume);
    hexter_engine_set_polyphony(e, poly);
    hexter_engine_set_mono_mode(e, mono);
    if (program < 1) program = 1;
    if (program > 128) program = 128;
    hexter_engine_select_program(e, program - 1);

    if (midi) {
        if (!load_midi_file(midi, &events, &end_time)) { hexter_engine_free(e); return 1; }
...

    {
        ...
        size_t next = 0;
        float buf[BLOCK];
        hexter_event_t evs[256];
        double peak = 0.0;

        if (!f) { fprintf(stderr, "cannot write %s\n", out_path); hexter_engine_free(e); return 1; }
        ...
        while (done < total) {
            ...
            while (next < events.n && ne < 256) {
                ...
                if (s >= done + n) break;
                {
                    ...
                    if (te->len)
                        hexter_event_from_midi(te->msg, te->len, frame, &evs[ne]);
                    else
                        hexter_event_from_midi(te->sysex, te->sysex_len, frame, &evs[ne]);
                    if (evs[ne].type != HEXTER_EV_NONE) ne++;
                }
                next++;
            }

            hexter_engine_render(e, buf, n, evs, ne);
            for (k = 0; k < n; k++) {
                float x = buf[k];
                if (x > 1.0f) x = 1.0f;
                if (x < -1.0f) x = -1.0f;
                if (fabs(x) > peak) peak = fabs(x);
                pcm[k] = (int16_t)lrintf(x * 32767.0f);
            }
            for (k = 0; k < n; k++) wr_le16(f, (uint16_t)pcm[k]);
            done += n;
        }
        fclose(f);
